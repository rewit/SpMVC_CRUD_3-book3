...
public class MemberController {
	...
	public String login(@RequestParam String m_userid,
		 ...
		 * 재 로그인을 수행할 때
		 * 정상 사용자가 아닐경우 memberVO가 null이 될 것이고
		 * 그로인해 session에 담긴 값이 null이 될 것이다
		 * 그러면 사용자는 자동으로 logout상태가 되어 보안 유지에
		 * 다소 유리하게 된다.
		 */
		MemberVO memberVO = null;
		
		if(bUser) memberVO = mService.findByUserId(m_userid);
	 ...
	 *   server는 요청에 맞는 결과는 response(응답)을 하게 되고
	 ...
	 * 		가. 로그인한 사용자가 누구인가
	 * 		나. 정상적인 로그인 사용자인가
	 * 		다. 로그인한 사용자가 원하는 reg가 정상적인 권한을 갖는가
	 * 		라. 이러한 정보를 바탕으로 사용자의 reg를 처리할 것인가
	 * 
	 ...
	 *   client에게 보내주고 이후에 client는 request정보에
	 *   이 session id를 같이 포함하여 서버로 요청을 보낸다.
	 *   
	 * 6.그러면 서버는 request 정보에 session id가 있는지를 확인하고
	 * 	 그 id정보를 기준으로하여 login된 정보를 취급하게 된다.
	 * 
