...
public class BookController {
	...
	public String book_input(@ModelAttribute BookVO bookVO,Model model) {
		 ...
		 * BOOKS에 담고 BODY에는 "BOOK-LIST"에 담아서 
...
//		model.addAttribute("BODY","BOOK-LIST");
		/*
		 * 코드를 작성하고 보니 이미 리스트를 보는 메서드가 존재한다
		 * 그래서 그 메서드를 호출하여 같은 일을 수행하려고 했더니
		 * 문제가 발생
		 * Spring  환경에서는 그럴경우 response를 통해 웹브라우저에게
		 * 해당 url을 request하도록 요청 할 수 있다
		 * 그 키워드가 redirect:....이다
		 */
		if(ret > 0 ) { 	return "redirect:book-view";
		}else {
...
// redirect:book-view
//  - 브라우저에 주소창에 .../book-view 라고 입력하고 enter를 눌러달라	요청
	
	}
	/*
	 * request의 변수를 수신하는 방법
	 * url?변수=값 형태로 전달된 request는
	 * 변수 이름과 같은 형태의 매개변수를 지정하여 받는다
	...
	/*
	 * update 수행1
	 * 1. 전달된 b_seq 값으로 데이터를 추출하고
	 * 2. 그 데이터를 입력 form에 전달하고
	 * 3. 입력form에서 데이터를 보면서 편집할 수 있도록 한다.
	 */
