"use strict"
//지금부터 철저한 문법규칙을 준수하겠다 선언
//기본문법과 런타임 동작을 모두 검사하여
//실수를 에러로 변환시키고
//변수 사용을 단수화 할 수 있도록 선언하는 것

//var num1 = 10
//let num2 = 20
//name = "홍길동" : JS 표준에는 사용할 수 있는 문법이지만 가급적 사용을 지양

$(function(){
	
	$("input").focus(function(){
		$(this).select()
	})
	
	
	$("#btn-save").click(function(){
		let s_title = $("#b_title").val()
		let s_comp = $("#b_comp").val()
		let s_auth = $("#b_auth").val()
		let s_price = $("#b_price").val()

		
		let msg = "도서명 : " +s_title + "\n"
		msg += "출판사 : " + s_comp + "\n"
		msg += "저자 : " + s_auth + "\n"
		msg += "가격 : " + s_price
		
		/*
		 * 	값의 유효성 검사
		 * 1. 입력값이 있는지
		 * 2. 입력한 값이 규칙에 맞는지
		 * 3. 모두 통과 했을 때만 서버로 데이터를 보낸다
		 * 
		 * 입력값을 전송하기 전에 유효성 검사
		 * 1. 데이터를 서버로 보내서 유효성 검사를 할수도 있찌만
		 * 	  최소한 원칙에 맞는 데이터를 사전에 검사하여
		 *   서버가 할 일을 줄여주자 하는 취지
		 * 2.사용자 입장에서 내가 입력한 값을 확인하면서
		 *   잘 입력되었는가를 알려주는 역할
		 */
		// 값이 입력되지 않은 항목이 있는지 검사
		if(s_title == ""){
			alert("도서명은 반드시 입력해야 합니다")
			$("#b_title").focus()
			//입력자에게 경고를 보여주고
			//해당 항목을 입력 할 수 있도록
			//커서를 입력박스에 생성시켜준다
			return false;
			//현재 항목의 유효성 검사를 하고 
			//통과하지 못했으므로 다음으로 진행 금지
		}
		if(s_comp == ""){
			alert("출판사는 반드시 입력해야 합니다")
			$("#b_comp").focus()
			return false;
		}
		if(s_auth == ""){
			alert("저자는 반드시 입력해야 합니다")
			$("#b_auth").focus()
			return false;
		}
		if(s_price == ""){
			alert("가격은 반드시 입력해야 합니다")
			$("#b_price").focus()
			return false;
		}
		
		if(isNaN(s_price)){
			alert("가격은 숫자로만 입력해야 합니다")
			
			/*
			 * 입력자의 입력을 도와주는 기능
			 * 기존의 값을 삭제하고 다시 입력할때 편의를 주는 기능
			 * 보통 type=password인 경우 .val("")
			 * 		type=text 인 경우 보통 .select()
			 */
			//$("#b_price").val("")
			//입력박스에 담긴 값을 지워라
			
			$("#b_price").select()
			//원래 입력된 text를 모두 감싸고
			//아무 text나 입력하면 원래 text를 삭제할 수 있도록
			$("#b_price").focus()
			return false;
		}
		//모든것이 완료됬으니 데이터를 서버로 보내라
		$("form").submit();
		
	})
})

